...
# Installing Debian  with reserved proxy and prebuild packages #

1. install Debian
2. install packages  

```
sudo apt install ca-certificates sudo unzip wget python python-pip
```
...
```
wget https://github.com/certbot/certbot/archive/v0.33.1.zip
unzip v0.33.1.zip
cd certbot-0.33.1/
...
server {
    ...
    listen 443;
    server_name ucode.pw;
 
    ssl_certificate           /etc/letsencrypt/live/ucode.pw/fullchain.pem;
    ssl_certificate_key       /etc/letsencrypt/live/ucode.pw/privkey.pem;
    ...
    location / {
      ...
      proxy_redirect      http://localhost:8008 https://ucode.pw;
    }
  }
```
check if ok, if not check config
```
sudo nginx -t
```
if ok enter this
